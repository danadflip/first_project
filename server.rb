...
# new review pages separated by fanciness

    # get('/reviews/q/:fanciness') do
    #   ids = $redis.lrange("review_ids", 0, -1).reverse
    #   @fanciness = []
    #   ids.each do |id|
    #     if $redis.hget("review:#{id}", "fanciness") == params[:fanciness]
...
# # posts each review, displaying the following params for each review_id
#     post("/reviews") do
#       id = $redis.incr("review_id")
#       $redis.hmset("review:#{id}", review_hash_from_params)
#       $redis.rpush("review_ids", id)
#       redirect to("/reviews")
#     end
# # TODO: instead of using restaurant names in url, use restaurant ids - redis
# # FIXME: delete not running properly, shows button but leads to nowhere
# # deletes individual restaurant review
#     delete "/reviews/r/:restaurant" do
#       ids = $redis.lrange("review_ids", 0, -1).reverse
#       rest_id = ids.select do |id|
#         $redis.hget("review:#{id}", "restaurant") == params[:restaurant]
#       end
#       $redis.del("review:#{rest_id.first}")
#       redirect("/reviews")
#     end

# # TODO:
# # new review page specific to restaurant
#     # get('/reviews/n/:restaurant') do
#     #   ids = $redis.lrange("review_ids", 0, -1).reverse
#     #   @restaurant = []
#     #   ids.each do |id|
#     #     if $redis.hget("review:#{id}", "restaurant") == params[:restaurant]
...
#     get("/reviews/r/:restaurant") do
#       ids = $redis.lrange("review_ids", 0, -1).reverse
#       restaurant_id = ids.select do |id|
#         $redis.hget("review:#{id}", "restaurant") == params[:restaurant]
#       end
#       @restaurant = $redis.hgetall("review:#{restaurant_id.first}")
#       render :erb, :show_restaurant, layout: :default
#     end
